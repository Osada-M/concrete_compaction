...
aveimg_path = '/media/nagalab/Volume01/kojima_ws/concrete_compaction/aveimg.jpg'
...
if yml["testresourcedata"]["readdata"] == "text" or yml["testresourcedata"]["readdata"] == "TEXT":
    if yml["SemanticSegmentaiton"]["available"]:
        ...
        testdata = dataset.new_style_text_dataset(yml["testresourcedata"]["resourcepath"])
...
# model = createmodel.create_model(choice_model_name='cosface', num_feature=2)
# model = createmodel.create_model(choice_model_name='nin_sphereface
# model = createmodel.create_model(choice_model_name='nin')
...
plot_model(model,yml["TESTModel"]["path"] + "./plot_model.png", show_shapes=True)
...
for count, (x, ans, fresh) in enumerate(zip(tqdm(images), labels, freshes)):
    # image file open.
    try:
    ...
    # predict /= np.linalg.norm(predict, axis=1, keepdims=True)
...
#動画データの番号
        sample_day = image_name.split('_')[0]   #動画番号の何日目か
        sample_num = image_name.split('_')[1]   #動画番号が何番目か

#画像データの名前を０１から２４でループさせる（インデックスにするために0から23にする）
...
with open(csv_path) as f:
    reader = csv.reader(f)
    header = next(reader)
    #csvファイルの画像データのリストの作成
    for row in tqdm(reader):
        video_num = day_num_video_list(row)     #数字だけじゃないから集合にしてもバラバラに保存される
        #print(video_num)
    ...
    reader = csv.reader(f)
    header = next(reader)
    split_result = []   #split_resultをmesh_set_listの長さだけのリストにする 例）長さが２の場合[[],[]]
